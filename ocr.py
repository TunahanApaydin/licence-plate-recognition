...
class OCR(object):
    ...
    def preprocess(self, frame, tracking_results: zip):
        tracking_results = list(tracking_results)
        if tracking_results:
            for (bbox, track_id, track_score) in (tracking_results):
                x1, y1, w, h = bbox
                if x1 < 0:
                    x1 = 0
                if y1 < 0:
    ...
    def postprocess(self, ocr_results, ocr_input):
        licence_plate_texts = []
        if ocr_results:
